...
export const handler = async (event: APIGatewayEvent) => {
  ...
  if (event.pathParameters && event.pathParameters.eventId) {
    ...
    if (!isSwimmingEvent(eventId)) {
      log.error(
        errorStatus("BADQUERY", "Invalid path parameter"),
      ...
      return Promise.resolve(
        response.error(
          400,
          {},
          new Error(`${eventId} is not a valid swimming eventId`)
        )
      );
    }
  } else {
    ...
    log.error(
      errorStatus("BADQUERY", "swimming eventId not specified"),
  ...
  log.info(
    httpStatus(200),
    `Success! Results for ${eventId}: ${JSON.stringify(items)}`
  );
  return Promise.resolve(
    response.success(
      200,
      {},
      { message: `Success! Results for ${eventId}: ${JSON.stringify(items)}` }
    )
  );
};